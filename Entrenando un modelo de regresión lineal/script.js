...
async function getData() {
  ...
  // const datosCasasR = await fetch('datas.json');
...
async function entrenarModelo(model, inputs, labels) {
  ...
  // como seva entrenando
  const surface = { name: 'show.history live', tab: 'Training' };
  // numero de registros que va a incluir
  const tamanioBatch = 28;
  // numero de veses que va a repetirse el modelo
  const epochs = 50;
  // nos va ayudar a mantener las metricas dureante el entrenamiento
  const history = [];
